...
def build_embedding_loader(
    model: nn.Module, cfg: Config, device: torch.device
...
def extract_embeddings(
    model: nn.Module, loader: DataLoader, device: torch.device, cfg: Config, *, progress: bool = True
) -> Tensor:
    if not hasattr(model, "forward_features") or not hasattr(model, "forward_head"):
        raise TypeError("The configured model must provide forward_features/forward_head.")

    amp_enabled = cfg.runtime.use_amp and device.type == "cuda"
    chunks: list[Tensor] = []
    samples_seen = 0
    for batch_index, (images, _, sample_indices) in enumerate(loader, start=1):
        expected_indices = torch.arange(samples_seen, samples_seen + sample_indices.numel())
        if not torch.equal(sample_indices, expected_indices):
            raise RuntimeError("Embedding loader must preserve sequential dataset indices.")
        ...
        if embeddings.ndim != 2:
            raise ValueError(f"Expected two-dimensional embeddings, got {embeddings.shape}.")
...
def _feature_chunk(feature_bank: Tensor, start: int, end: int, device: torch.device) -> Tensor:
    chunk = feature_bank[start:end]
    return chunk if chunk.device == device else chunk.to(device)


@torch.inference_mode()
def build_exact_global_knn(embeddings: Tensor, cfg: Config, device: torch.device) -> tuple[Tensor, Tensor]:
    if embeddings.ndim != 2:
        raise ValueError(f"embeddings must be [N, D], got {embeddings.shape}.")
    sample_count = embeddings.size(0)
    k = cfg.global_knn.k
    if sample_count <= k:
        raise ValueError(f"Global KNN requires more than k={k} samples; found {sample_count}.")
    ...
    for query_start in range(0, sample_count, query_chunk_size):
        ...
        best_squared_distances = torch.full((query_count, k), float("inf"), device=device)
        best_indices = torch.full((query_count, k), -1, dtype=torch.long, device=device)
        ...
        for reference_start in range(0, sample_count, reference_chunk_size):
            ...
            references = _feature_chunk(feature_bank, reference_start, reference_end, device)
            reference_norms = references.square().sum(dim=1).unsqueeze(0)
            squared_distances = (
                query_norms + reference_norms - 2.0 * queries @ references.transpose(0, 1)
            ).clamp_min_(0)
        ...
        index_for_bank = best_indices if feature_bank.device == device else best_indices.cpu()
...
def save_artifact(
    ...
    if output_path.exists() and not cfg.global_knn.overwrite:
        raise FileExistsError(
            f"Global KNN artifact already exists: {output_path}. Set global_knn.overwrite=True to replace it."
        )
    ...
    preprocessing = evaluation_preprocessing_signature(cfg.data, cfg.model.name)
    provenance = {
...
def main(cfg: Config = CFG) -> None:
    ...
    model = load_warmup_backbone(cfg, device, trainable=False)
    ...
    if embeddings.size(0) != len(dataset):
        raise RuntimeError(f"Embedding count mismatch: {embeddings.size(0)} != {len(dataset)}.")
    del loader
    release_accelerator(model, device)

    indices, cosine = build_exact_global_knn(embeddings, cfg, device)
    artifact_path = save_artifact(dataset, embeddings, indices, cosine, cfg)
