...
Runtime:
    ...
    - Outputs are separated automatically by ACTOR_UPDATE_MODE.
...
OUTPUT_DIR = (
    PROJECT_ROOT
    / "outputs"
    / f"mnist_test_rtx5080_{ACTOR_UPDATE_MODE}"
)
...
def _save_rl_checkpoint(
    ...
    *,
    ...
    """Atomically save the actor and metadata needed for model selection."""
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary_path = path.with_suffix(f"{path.suffix}.tmp")
    validation_metrics = {
