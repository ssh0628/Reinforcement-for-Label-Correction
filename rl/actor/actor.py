...
class PolicyActor(nn.Module):
    ...
    def __init__(self, feature_extractor: nn.Module, cfg: Config) -> None:
        super().__init__()
        if not hasattr(feature_extractor, "forward_features") or not hasattr(
            feature_extractor, "forward_head"
        ):
            raise TypeError("feature_extractor must provide forward_features/forward_head.")
    ...
    def encode(self, images: Tensor) -> Tensor:
        ...
        embeddings = self.feature_extractor.forward_head(feature_map, pre_logits=True)
        if embeddings.ndim != 2:
            raise ValueError(f"Expected two-dimensional embeddings, got {embeddings.shape}.")
        return embeddings


def load_policy_actor(cfg: Config, device: torch.device) -> PolicyActor:
    feature_extractor = load_warmup_backbone(cfg, device, trainable=True, remove_classifier=True)
