...
def _validate_inputs(embeddings: Tensor, k: int, query_chunk_size: int, reference_chunk_size: int) -> None:
    ...
    if embeddings.size(0) <= k:
        raise ValueError(f"Policy KNN requires more than k={k} samples; found {embeddings.size(0)}.")
...
def build_exact_policy_knn(
    ...
    _validate_inputs(embeddings, k, query_chunk_size, reference_chunk_size)
    sample_count = embeddings.size(0)
    device = embeddings.device
    neighbor_indices = torch.empty((sample_count, k), dtype=torch.long, device=device)
    ...
    for query_start in range(0, sample_count, query_chunk_size):
        ...
        best_distances = torch.full((query_count, k), float("inf"), device=device)
        ...
        if torch.any(best_indices < 0) or torch.any(~torch.isfinite(best_distances)):
