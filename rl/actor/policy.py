...
def _validate_policy_batch(
    ...
    if query_embeddings.ndim != 2:
        raise ValueError(f"query_embeddings must be [B, D], got {query_embeddings.shape}.")
    if neighbor_embeddings.ndim != 3:
        raise ValueError(f"neighbor_embeddings must be [B, K, D], got {neighbor_embeddings.shape}.")
    if current_labels.ndim != 2:
        raise ValueError(f"current_labels must be [B, C], got {current_labels.shape}.")
    if neighbor_labels.ndim != 3:
        raise ValueError(f"neighbor_labels must be [B, K, C], got {neighbor_labels.shape}.")
    ...
    neighbor_label_batch, label_neighbor_count, label_classes = neighbor_labels.shape
    ...
    if not current_labels.is_floating_point() or not neighbor_labels.is_floating_point():
...
class LabelCorrectionPolicy(nn.Module):
    ...
    def compute_attention(self, query_embeddings: Tensor, neighbor_embeddings: Tensor) -> Tensor:
    ...
    @staticmethod
    def predict_neighbor_labels(attention_weights: Tensor, neighbor_labels: Tensor) -> Tensor:
        return torch.einsum("bk,bkc->bc", attention_weights, neighbor_labels.float())

    @staticmethod
    def compute_correction_probabilities(predicted_labels: Tensor, current_labels: Tensor) -> Tensor:
        ...
        probabilities = numerator / denominator.clamp_min(torch.finfo(predicted_labels.dtype).tiny)
        return probabilities.clamp(0.0, 1.0)

    @staticmethod
    def _sample_or_validate_actions(probabilities: Tensor, actions: Tensor | None) -> Tensor:
        if actions is None:
            return torch.bernoulli(probabilities).to(torch.bool)
        if actions.shape != probabilities.shape:
            raise ValueError(f"actions must have shape {probabilities.shape}, got {actions.shape}.")
    ...
    def forward(
        ...
        *,
        ...
        _validate_policy_batch(query_embeddings, neighbor_embeddings, current_labels, neighbor_labels)
        current_labels = current_labels.float()
        attention_weights = self.compute_attention(query_embeddings, neighbor_embeddings)
        predicted_labels = self.predict_neighbor_labels(attention_weights, neighbor_labels)
        probabilities = self.compute_correction_probabilities(predicted_labels, current_labels)
        sampled_actions = self._sample_or_validate_actions(probabilities, actions)

        probability_epsilon = torch.finfo(probabilities.dtype).eps
        stable_probabilities = probabilities.clamp(min=probability_epsilon, max=1.0 - probability_epsilon)
        log_probabilities = torch.where(
            sampled_actions, stable_probabilities.log(), torch.log1p(-stable_probabilities)
        )
    ...
    def correct_all(self, embeddings: Tensor, labels: Tensor, neighbor_indices: Tensor) -> CorrectionResult:
        ...
        if neighbor_indices.ndim != 2:
            raise ValueError(f"neighbor_indices must be [N, K], got {neighbor_indices.shape}.")
        ...
        if torch.any(neighbor_indices < 0) or torch.any(neighbor_indices >= sample_count):
            raise ValueError("neighbor_indices contains an out-of-range index.")

        device = embeddings.device
        probabilities = torch.empty(sample_count, dtype=torch.float32, device=device)
        ...
        return CorrectionResult(
            correction_probabilities=probabilities, actions=actions, corrected_labels=corrected_labels
        )
