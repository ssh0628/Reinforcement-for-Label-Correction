...
----------------
Data:
    - MNIST train subset: 10,000 images, balanced to 1,000 per digit.
...
Backbone and supervised warmup:
    ...
    - 10 warmup epochs using only noisy labels for optimization.
    - Epochs 1-2: classifier head only, AdamW lr=1e-3.
    - Epochs 3-10: backbone lr=1e-4 and head lr=5e-4.
...
FULL_MNIST_TRAIN_SAMPLES = 60_000
TRAIN_SAMPLES_PER_CLASS = 1_000
EXPECTED_SAMPLES = TRAIN_SAMPLES_PER_CLASS * NUM_CLASSES
NOISE_RATE = 0.40
...
def load_full_mnist_train() -> tuple[Tensor, Tensor]:
    ...
    images = dataset.data.contiguous()
    labels = dataset.targets.to(torch.long).contiguous()
    if (
        images.size(0) != FULL_MNIST_TRAIN_SAMPLES
        or labels.size(0) != FULL_MNIST_TRAIN_SAMPLES
    ):
        raise RuntimeError(
    ...
    for digit in DIGITS:
        class_indices = labels.eq(digit).nonzero(as_tuple=False).flatten()
        if class_indices.numel() < TRAIN_SAMPLES_PER_CLASS:
            raise RuntimeError(
                f"Digit {digit} has only {class_indices.numel()} samples, "
                f"but {TRAIN_SAMPLES_PER_CLASS} were requested."
            )
        ...
        selected_chunks.append(
            class_indices[permutation[:TRAIN_SAMPLES_PER_CLASS]]
        )
    selected = torch.cat(selected_chunks)
    selected = selected[
        torch.randperm(selected.numel(), generator=generator)
    ]
    images = pin_for_cuda(images[selected].contiguous())
    labels = pin_for_cuda(labels[selected].contiguous())
    if images.size(0) != EXPECTED_SAMPLES:
        raise RuntimeError(
            f"Local MNIST subset has {images.size(0)} samples; "
...
def print_configuration(
    ...
    print(
        ...
        f"knn_mode=full_{clean_labels.numel()}x{clean_labels.numel()}"
    )
    print(
