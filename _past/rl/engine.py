...
from cifar_test.setting.data import (
    CIFAR10_MEAN,
    CIFAR10_STD,
    EXPECTED_SAMPLES,
    MODEL_NAME,
...
ACTOR_STEP_MODE = CONFIG.rl.actor_step_mode
USE_REMAINING_HORIZON = CONFIG.rl.use_remaining_horizon
USE_TERMINAL_CRITIC_UPDATE = CONFIG.rl.use_terminal_critic_update
POLICY_UPDATE_SAMPLES = CONFIG.actor_update_samples
POLICY_UPDATE_BATCH_SIZE = CONFIG.actor_update_batch_size
RECORD_CHANGE_DIAGNOSTICS = CONFIG.rl.record_change_diagnostics
CHANGE_DIAGNOSTIC_PROBE_SIZE = CONFIG.change_diagnostic_probe_size
...
def _save_rl_checkpoints(
    ...
    *,
    ...
    actor_payload = {
        ...
        "actor_step_mode": ACTOR_STEP_MODE,
        "actor_batch_size": POLICY_UPDATE_BATCH_SIZE,
...
def print_configuration(device: torch.device, sample_count: int) -> None:
    ...
    print(
        ...
        f"actor_step={ACTOR_STEP_MODE} batch={POLICY_UPDATE_BATCH_SIZE} actor_lr={ACTOR_LR}"
    )
...
def main() -> None:
    ...
    measure(
        ...
        lambda: warm_device_kernels(
            model, raw_images, device, mean, std, update_batch_size=POLICY_UPDATE_BATCH_SIZE
        ),
    )
    ...
    for epoch in range(1, RL_EPOCHS + 1):
        ...
        for step in range(1, TRAJECTORY_LENGTH + 1):
            ...
            query_indices_cpu = select_queries(
                ...
                SEED,
                global_step,
                shuffle=ACTOR_STEP_MODE == "minibatch",
            )
            actor_loss = measure(
                "actor_update",
                device,
